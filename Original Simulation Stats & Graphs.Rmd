...
We then changed two other parameter values in the original simulation, the mean in the normal distribution for running plays and the mean in the log-normal distribution for passing plays. The run mean takes on values 2, 3, and 4; as noted previously, a mean of 3 was in the original simulation study. The pass mean takes values log (5), log (10), and log (15) with the middle value, log (10), the original simulation parameter. Finally, the pass completion base that was originally set to .97 is also tested at values .9 and .95. Given the three parameter values for each three parameters, we ran twenty-seven different combinations in a series of simulations and recorded the probability to make a touchdown and the average number of plays given a touchdown was made. Again, all trials were simulated 10,000 times. The following data frame labeled Figure 5 shows the output of all the combinations and their results. The highest values for both probability and number of plays were recorded when the mean run was set to four and the pass completion base was set to .97. The different pass means in the log- normal distribution did not largely affect the outcome. Since this value only returns the number for yardage before knowledge if the pass was completed or not, the performance of this parameter makes sense. 
...
To graphically represent this data, the following plots, Figure 6a and 6b, depict the results found Figure 5. As the run mean increases along the x axis, both the touchdown probability and average play count generally increase as well. Although, the average play count (given a touchdown) does not increase as distinctly. The shapes of the points correspond to the different pass completion bases and the colors correspond to the different pass mean values as denoted to the legends to the right. At almost all instances, the blue or log (5) pass mean will have the slightly higher probability when keeping the other parameters constant in that set of simulations. Likewise, the square shaped points or the .97 pass completion base returns the highest results when keeping the other parameters constant.   
```{r, echo = FALSE}

ggplot(data = emp.data, aes(x = run_mean, y = prob_td, colour = pass_mean)) + geom_point(aes(shape = factor(p_comp_base)))+ggtitle("Figure 6a: Touchdown Probability Simulations") + 
...
ggplot(data = emp.data, aes(x = run_mean, y = avrg_plays, colour = pass_mean)) + 
  geom_point(aes(shape = factor(p_comp_base)))+ggtitle("Figure 6b: Average Play Count (Given TD) Simulations") + 
...
***Different Parameter Simulations***
```{r, eval = FALSE}
